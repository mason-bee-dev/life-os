-- Record logged-at timestamps for WP / sleep, and preserve drink created_at on rewrite.

alter table public.daily_records
  add column if not exists wp_logged_at timestamptz;

comment on column public.daily_records.wp_logged_at is
  'When WP fields were last logged for this date';

alter table public.sleep_records
  add column if not exists night_logged_at timestamptz,
  add column if not exists nap_logged_at timestamptz;

comment on column public.sleep_records.night_logged_at is
  'When night sleep was last logged for this date';
comment on column public.sleep_records.nap_logged_at is
  'When nap was last logged for this date';

-- Backfill from existing row timestamps where domain data exists.
update public.daily_records
set wp_logged_at = coalesce(wp_logged_at, updated_at)
where wp_logged_at is null
  and (
...
set night_logged_at = coalesce(night_logged_at, updated_at, created_at)
where night_logged_at is null
  and bedtime is not null
  and wake_time is not null;

update public.sleep_records
set nap_logged_at = coalesce(nap_logged_at, updated_at, created_at)
where nap_logged_at is null
