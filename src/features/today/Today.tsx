  ...
  return (
    <div className={card + " " + className}>
      <div className="mb-4 flex items-center gap-2">
        <span className="grid h-[26px] w-[26px] place-items-center rounded-lg" style={{ color, background: `color-mix(in srgb, ${color} 12.5490196078%, transparent)` }}>
          <Icon size={15} />
        </span>
...
export function Today({ habits, toggle, addEntry }: TodayProps) {
  ...
  const energyFill = { background: `linear-gradient(90deg,var(--primary) ${energy}%,var(--track) ${energy}%)` };
  const sleepPct = ((sleep - 4) / 6) * 100;
  const sleepFill = { background: `linear-gradient(90deg,var(--metric-sleep) ${sleepPct}%,var(--track) ${sleepPct}%)` };
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <Field icon={Smile} color="var(--metric-mood)" title="Hôm nay bạn cảm thấy thế nào?">
            ...
            {moodFaces.map((f, i) => (
              <button
                ...
                className={
                  ...
                  (mood === i + 1 ? "border-violet-500 bg-violet-500/10" : "border-border hover:border-border-hover")
                }
              >
                <span className={"text-2xl transition-transform " + (mood === i + 1 ? "scale-110" : "grayscale-[0.4]")}>{f}</span>
                <small className={"text-[11px] " + (mood === i + 1 ? "text-violet-400" : "text-faint")}>
                  {moodLabels[i]}
                </small>
              </button>
            ))}
          ...
          <div className="mb-3 flex justify-between text-3xl">
            ...
            <span className="text-body">{energy}<span className="text-[12.5px] text-faint">/100</span></span>
          </div>
        ...
        <Field icon={Moon} color="var(--metric-sleep)" title="Giấc ngủ" right={<span className="text-[15px] font-bold">{sleepStr}</span>}>
        ...
        <Field icon={Scale} color="var(--destructive)" title="Cân nặng" right={<span className="text-[15px] font-bold">{weight.toFixed(1)} <span className="text-sm font-medium text-faint">kg</span></span>}>
          <div className="flex items-center justify-center gap-5 py-1.5">
            ...
            <div className="min-w-[120px] text-center text-3xl tracking-tight">
              {weight.toFixed(1)}<small className="ml-1 text-sm text-faint">kg</small>
            </div>
            <button onClick={() => setWeight((w) => +(w + 0.1).toFixed(1))}
              className="grid h-[42px] w-[42px] place-items-center rounded-xl border border-border hover:border-primary hover:text-primary">
              <Plus size={16} />
        ...
        <Field icon={GlassWater} color="var(--metric-water)" title="Nước" right={<span className="text-[15px] font-bold">{(glasses * 0.25).toFixed(2)} <span className="text-[12.5px] text-faint">L</span></span>}>
          ...
          <div className="mt-2.5 text-[11px] text-faint">Mục tiêu: 2.0 L · chạm vào từng ly để ghi lại</div>
        </Field>
        ...
        <Field icon={PenLine} color="var(--primary)" title="Hôm nay của bạn thế nào?" className="md:col-span-2">
          <textarea
            value={note}
          ...
          />
          <div className="text-[11px] text-faint">Ghi chú sau khi lưu sẽ tự động trở thành một mục trong Nhật ký.</div>
        </Field>
      </div>
