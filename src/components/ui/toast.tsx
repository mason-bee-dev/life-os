...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ notify }}>
      {children}
      {message && (
        <div className="fixed top-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-xl bg-[var(--toast)] px-5 py-3 text-sm text-[var(--toast-foreground)] shadow-[0_10px_30px_var(--shadow-color-35)]">
